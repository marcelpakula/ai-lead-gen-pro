...
SZYBKOSC STRONY (Google PageSpeed, mobile, 0-100): {wer["pagespeed"] if wer["pagespeed"] is not None else "nie sprawdzono"}
...
- Dodaj do <html> klase "scroll-smooth" (Tailwind). Na koniec <body> dodaj <script> z fallbackiem: dla kazdego linku nav (querySelectorAll('nav a[href^="#"]')) dodaj addEventListener('click') ktory robi event.preventDefault() i document.querySelector(href).scrollIntoView({behavior:'smooth', block:'start'}) - to gwarantuje dzialajaca nawigacje nawet w iframe.
